...
# 1. 옵션 처리
# =========================
parser = argparse.ArgumentParser(description="Daily news summary sender")
parser.add_argument("--test", action="store_true", help="테스트용: 최근 24시간 기준 조회")
...
if args.test:
    ...
    end_dt = datetime.now()
    print(" 테스트 모드 : 최근 24시간 기준 조회")
else:
    # 운영용: 오전 6시 기준 메일 발송
    today = datetime.now().date()
    #yesterday = datetime.now() - timedelta(days=1)
    start_dt = datetime.combine(today, datetime.min.time()) # 오늘 0시
    #end_dt = start_dt + timedelta(days=1)
    end_dt = datetime.now() # 현재 시각 (메일 발송 시점)
    print(f" 운영 모드: {start_dt.strftime('%Y-%m-%d %H:%M')} ~ {end_dt.strftime('%H:%M')} 조회")
...
# 9. 메일 발송
# =========================
subject = f"[news_crawler] {start_dt.date()} 수집 요약"

proc = subprocess.Popen(
    ["mail", "-s", subject, "vjihyun.bangv@gmail.com"],
